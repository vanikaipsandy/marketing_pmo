    ...
    <UserLayout title="Detail Review PC">
        <div class="space-y-6 animate-fade-in-up">
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#171717]">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <Link
                        ...
                        class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-sky-300 hover:bg-sky-50 hover:text-sky-700 dark:border-white/20 dark:bg-[#1f1f1f] dark:text-slate-200 dark:hover:border-sky-500/60 dark:hover:bg-sky-900/30 dark:hover:text-sky-200"
                    >
                        Kembali
                    </Link>
                </div>
            ...
            <section class="space-y-4">
                <div class="bg-[#1661ad] px-4 py-1.5 text-sm font-semibold text-white">
                    Kesimpulan / Hasil Review
                </div>
                <article class="border border-[#9ec6e7] bg-white px-6 py-4 dark:border-sky-600/40 dark:bg-[#171717]">
                    <h2 class="text-[2rem] font-extrabold leading-tight text-slate-900 dark:text-white">
                        {{ review.kesimpulan || '-' }}
                    </h2>
                    <p class="mt-2 text-base font-medium text-slate-700 dark:text-slate-200">
                        {{ review.detail_kesimpulan || '-' }}
                    </p>
                    <ul v-if="penjelasanItems.length" class="mt-4 list-disc space-y-2 pl-6 text-base leading-relaxed text-slate-800 dark:text-slate-200">
                        <li v-for="(item, index) in penjelasanItems" :key="`penjelasan-${index}`">{{ item }}</li>
                    </ul>
                    <p v-else class="mt-4 whitespace-pre-line text-base leading-relaxed text-slate-800 dark:text-slate-200">
                        {{ review.penjelasan || '-' }}
                    </p>
                </article>
            </section>
            ...
            <section class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                    <article class="border border-[#b7daf1] bg-white dark:border-sky-700/40 dark:bg-[#171717]">
                        <header class="bg-[#a8d0ed] px-4 py-2 text-2xl font-semibold text-slate-900 dark:bg-sky-900/40 dark:text-slate-100">
                            Why
                        </header>
                        <div class="px-4 py-4">
                            <ul v-if="whyItems.length" class="list-disc space-y-2 pl-5 text-base leading-relaxed text-slate-800 dark:text-slate-200">
                                <li v-for="(item, index) in whyItems" :key="`why-${index}`">{{ item }}</li>
                            </ul>
                            <p v-else class="whitespace-pre-line text-base leading-relaxed text-slate-800 dark:text-slate-200">
                                {{ review.why || '-' }}
                            </p>
                        </div>
                    </article>
                    ...
                    <article class="border border-[#b7daf1] bg-white dark:border-sky-700/40 dark:bg-[#171717]">
                        ...
                        <div class="px-4 py-4">
                            <p class="whitespace-pre-line text-base leading-relaxed text-slate-800 dark:text-slate-200">
                                {{ review.what || '-' }}
                            </p>
                        </div>
                    </article>
                    ...
                    <article class="border border-[#b7daf1] bg-white dark:border-sky-700/40 dark:bg-[#171717]">
                        ...
                        <div class="px-4 py-4">
                            <p v-if="howParsed.intro" class="mb-2 text-base leading-relaxed text-slate-800 dark:text-slate-200">
                                {{ howParsed.intro }}
                            </p>
                            <ol v-if="howParsed.steps.length" class="list-decimal space-y-2 pl-5 text-base leading-relaxed text-slate-800 dark:text-slate-200">
            ...
            <section class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                    <article class="rounded-xl border border-slate-200 bg-white px-4 py-4 dark:border-white/10 dark:bg-[#171717]">
                        <header class="mb-3 flex items-center gap-2">
                            ...
                            <h3 class="text-3xl font-extrabold text-slate-900 dark:text-white">Project Profile</h3>
                        </header>
                        <ul v-if="projectProfileItems.length" class="list-disc space-y-2 pl-5 text-base leading-relaxed text-slate-800 dark:text-slate-200">
                    ...
                    <article class="rounded-xl border border-slate-200 bg-white px-4 py-4 dark:border-white/10 dark:bg-[#171717]">
                        <header class="mb-3 flex items-center gap-2">
                            ...
                            <h3 class="text-3xl font-extrabold text-slate-900 dark:text-white">Key Milestone</h3>
                        </header>
                        <p class="whitespace-pre-line text-base leading-relaxed text-slate-800 dark:text-slate-200">
                    ...
                    <article class="rounded-xl border border-slate-200 bg-white px-4 py-4 dark:border-white/10 dark:bg-[#171717]">
                        <header class="mb-3 flex items-center gap-2">
                            ...
                            <h3 class="text-3xl font-extrabold text-slate-900 dark:text-white">Risk &amp; Impact Value</h3>
                        </header>
                        <p class="whitespace-pre-line text-base leading-relaxed text-slate-800 dark:text-slate-200">
                            {{ review.risk_impact || '-' }}
                        </p>
