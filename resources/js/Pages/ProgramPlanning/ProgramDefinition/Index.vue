<template>
    <UserLayout title="Program Definition">
        <div class="animate-fade-in">
            ...
            <section class="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div class="flex h-full flex-col gap-3">
                    <article
                        role="button"
                        tabindex="0"
                        class="relative flex min-h-[220px] flex-1 cursor-pointer flex-col justify-center rounded-2xl border border-[#1C75BC] bg-[#1C75BC] p-5 shadow-[0_4px_16px_rgba(28,117,188,0.3)]"
                        @click="showDigitalMasterInitiatives"
                        @keydown.enter.prevent="showDigitalMasterInitiatives"
                        @keydown.space.prevent="showDigitalMasterInitiatives"
                    >
                        <p
                ...
                <article class="h-full min-h-[220px] space-y-4 rounded-2xl border border-slate-200 bg-white px-5 py-3 shadow-[0_4px_16px_rgba(0,0,0,0.05)] dark:border-white/10 dark:bg-[#171717] lg:col-span-2">
                    <div>
                    ...
                    <div class="border-t border-slate-100 pt-4 dark:border-white/5">
                        <div class="mb-2 flex items-center justify-between gap-2">
                            <h2 class="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-500 dark:text-slate-400">
                                Project Charter Digital Initiative Timeline
                            </h2>
                        </div>

                        <div>
                            <div class="grid" :style="gridStyle(projectSteps)">
                                <div
                                    v-for="(step, index) in projectSteps"
                                    :key="`project-step-${step.key}`"
                                    class="relative flex justify-center"
                                >
                                    <span
                                        class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold"
                                    ...
                                    <span
                                        v-if="index < projectSteps.length - 1"
                                        class="absolute left-1/2 top-1/2 ml-[0.75rem] h-0.5 w-[calc(100%_-_1.5rem)] -translate-y-1/2 rounded-full"
                                        :class="step.lineClass"
                                    ></span>
                                </div>
                            ...
                            <div class="mt-2 grid gap-1 text-center" :style="gridStyle(projectSteps)">
                                <div v-for="step in projectSteps" :key="`project-label-${step.key}`">
                                    <p class="text-[9px] font-semibold text-slate-700 dark:text-slate-200">{{ step.label }}</p>
                                </div>
                            </div>
                        </div>
            ...
            <InitiativeCards
                class="mb-4"
                :digital-total="totalDigitalInitiatives"
                :it-total="totalItInitiatives"
                @show-digital="showDigitalMasterInitiatives"
                @show-it="showItMasterInitiatives"
            />

            <DigitalMasterInitiativeTable
                v-if="tableMode === TABLE_MODE.DIGITAL_MASTER"
                :items="masterDigitalList"
                :initiative-items="initiativeItemsList"
            />

            <ItMasterInitiativeTable
                v-else-if="tableMode === TABLE_MODE.IT_MASTER"
                :items="masterItList"
            />
        </div>
    </UserLayout>
...
import InitiativeCards from '@/Components/ProgramPlanning/ProgramDefinition/InitiativeCards.vue';
...
const showDigitalMasterInitiatives = () => {
    tableMode.value = TABLE_MODE.DIGITAL_MASTER;
};

const showItMasterInitiatives = () => {
    tableMode.value = TABLE_MODE.IT_MASTER;
};
